...
const INSPECTION_CHECKLISTS = {
    "DAN-STM": {
        ...
        "monthly": [
            ...
            { category: "Flanges & Joint", item: "Aline Key Screws / bolts" },
            { category: "Rim / Tyres", item: "Tyre Pressure" },
            { category: "Rim / Tyres", item: "Tyre & Rim Condition" },
...
// --- BEGIN FIX ---
// Save the final state for the download functions BEFORE clearing it
...
if (basicInfo) {document.getElementById('areaLocation').value = basicInfo.areaLocation || '';
...
// Sync the frequency dashboard with stored selection
...
// Get the selected equipment and frequency
...
if (item) {document.getElementById('areaLocation').value = item.areaLocation || '';
...
doc.text(`Equipment: ${basicInfo.equipment} (${basicInfo.inspectionFrequency.charAt(0).toUpperCase() + basicInfo.inspectionFrequency.slice(1)})`, doc.internal.pageSize.getWidth() / 2, 28, { align: 'center' });
const summaryData = [
  ['Area / Location', basicInfo.areaLocation, 'Inspected By', basicInfo.inspectedBy],
  ['Tag Number', basicInfo.tagNo, 'Date of Inspection', basicInfo.inspectionDate],
  ['Time of Inspection', basicInfo.inspectionTime, 'Frequency', (basicInfo.inspectionFrequency || '').toUpperCase()]
];
...
const sanitizedTag = (basicInfo.tagNo || 'TAG').replace(/[^a-zA-Z0-9]/g, '_');
  const freq = (basicInfo.inspectionFrequency || 'inspection').toUpperCase();
  const fileName = `OMI_Report_${sanitizedEquipment}_${sanitizedTag}_${freq}_${basicInfo.inspectionDate}.pdf`;
doc.save(fileName);
} finally {
loadingOverlay.style.display = 'none';
}
...
wsSummary['A8'].s = summaryHeaderStyle;
wsSummary['B8'].s = summaryValueStyle;
wsSummary['!cols'] = [{ wch: 25 }, { wch: 40 }];
XLSX.utils.book_append_sheet(wb, wsSummary, "Inspection Summary");
// --- Details Sheet ---
let photoCounter = 1;
const detailsHeaders = ["Category", "Checklist Item", "Status", "Remarks", "Photo Attached"];
...
const rowNum = r + 2; // +1 for 0-index, +1 for header row
const status = detailsData[r][2]; // Get status value
...
if (status === 'OK') {
wsDetails[`C${rowNum}`].s = statusOKStyle;
} else if (status === 'NOT OK') {
wsDetails[`C${rowNum}`].s = statusNotOKStyle;
} else {
wsDetails[`C${rowNum}`].s = statusNAStyle;
}
}
...
const sanitizedTag = (basicInfo.tagNo || 'TAG').replace(/[^a-zA-Z0-9]/g, '_');
  const freq = (basicInfo.inspectionFrequency || 'inspection').toUpperCase();
  const fileName = `OMI_Report_${sanitizedEquipment}_${sanitizedTag}_${freq}_${basicInfo.inspectionDate}.xlsx`;
XLSX.writeFile(wb, fileName);
}
// ========================================================================
// END: MODIFIED EXCEL FUNCTION
// ========================================================================
...
function init() {
// No need to initialize checklist here - it will be loaded based on selections
